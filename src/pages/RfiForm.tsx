...
// Inline contact search dropdown for PIS contact sections
function PISContactSearchBox({
  token,
  sectionId,
  subSection,
  onSelect,
  ...
  const { results, loading } = usePISContactSuggestions(token, query);
  ...
  return (
    <div ref={wrapperRef} className="col-span-full relative mb-2">
      <div className="relative">
        ...
        <Input
          ...
          placeholder={`Search ${label} from contacts...`}
          className="pl-9 h-9 text-sm bg-amber-50/50 border-amber-200 focus:border-amber-500 focus:ring-amber-500/20"
        />
        {loading && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 animate-spin text-amber-500" />}
      </div>
      {open && query.trim().length >= 2 && results.length > 0 && (
        <div className="absolute left-0 right-0 top-full mt-1 z-[9999] rounded-md border bg-white shadow-lg max-h-[200px] overflow-y-auto">
          {results.map((r) => (
            <button
              key={`${r.type}-${r.id}`}
              type="button"
              className="w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-amber-50 text-left transition-colors"
              onClick={() => {
                const fields = mapContactToFields(r, sectionId, subSection);
                onSelect(fields);
                setQuery("");
                setOpen(false);
              }}
            >
              ...
              <div className="min-w-0">
                <div className="truncate font-medium text-stone-700">{r.name}</div>
                {r.company_name && r.company_name !== r.name && (
                  <div className="text-xs text-stone-400 truncate">{r.company_name}</div>
                )}
              </div>
            </button>
          ))}
...
export default function RfiForm() {
  ...
  // Determine which contact search boxes to show for a given section/field position
  const getContactSearchForField = (sectionId: string, fieldId: string): React.ReactNode | null => {
    if (!token && !isDemo) return null;
    if (isDemo) return null; // No search in demo mode
    
    const applyFields = (fields: Record<string, string>) => {
      setResponses(prev => ({ ...prev, ...fields }));
    };

    // Show search box right before the first detail field of each contact group
    if (sectionId === "contractors_inspections") {
      if (fieldId === "gc_name" && responses["contractors_inspections_gc_known"] === "Yes") {
        return <PISContactSearchBox token={token} sectionId={sectionId} subSection="gc" onSelect={applyFields} label="General Contractor" />;
      }
      if (fieldId === "sia_name" && responses["contractors_inspections_sia_known"] === "Yes") {
        return <PISContactSearchBox token={token} sectionId={sectionId} subSection="sia" onSelect={applyFields} label="Special Inspector" />;
      }
      if (fieldId === "tpp_name") {
        const tppKnown = responses["contractors_inspections_tpp_known"];
        if (tppKnown === "Yes") {
          return <PISContactSearchBox token={token} sectionId={sectionId} subSection="tpp" onSelect={applyFields} label="TPP Applicant" />;
        }
      }
    }
    ...
    if (sectionId === "applicant_and_owner" && fieldId === "applicant_first_name") {
      return <PISContactSearchBox token={token} sectionId={sectionId} subSection="applicant" onSelect={applyFields} label="Applicant" />;
    }
    return null;
  };
  ...
  const renderField = (field: RfiFieldConfig, sectionId: string, repeatIdx: number = 0) => {
    ...
    // Inject contact search box before certain fields
    const contactSearch = getContactSearchForField(sectionId, field.id);
                    ...
                    <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {fields.flatMap((field) => {
                          const search = getContactSearchForField(currentSection.id, field.id);
                          const rendered = renderField(field, currentSection.id);
                          return search ? [search, rendered] : [rendered];
                        })}
                      </div>
                    </div>
                  );
                  ...
                  <div className="space-y-6">
                    ...
                    {Array.from({ length: repeatCount }).map((_, repeatIdx) => (
                      <div key={repeatIdx} className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {repeatableFields.slice(1).flatMap((field) => {
                            const search = getContactSearchForField(currentSection.id, field.id);
                            const rendered = renderField(field, currentSection.id, repeatIdx);
                            return search ? [search, rendered] : [rendered];
                          })}
                        </div>
                      </div>
          ...
          {isReviewStep ? (
            <div className="flex items-center gap-2 ml-auto">
              ...
              <Button
                ...
                disabled={submitting}
                className="bg-emerald-600 hover:bg-emerald-500 text-white font-semibold px-6 h-11 rounded-xl disabled:opacity-50"
              >
                {submitting ? (
                  <>
