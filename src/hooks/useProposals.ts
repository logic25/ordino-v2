...
export function useSignProposalInternal() {
  ...
  return useMutation({
    mutationFn: async ({ id, signatureData, assignedPmId }: { 
      ...
      // Migrate proposal contacts → project contacts (find or create client_contacts, then link)
      try {
            ...
            if (!contactId) {
              ...
              const { data: newContact } = await supabase
                ...
                .insert({
                  ...
                  client_id: pc.client_id || (proposal.client_id || null),
                } as any)
                .select("id")
                .single();
      ...
      try {
        await supabase.from("universal_documents").insert({
          ...
          category: "contract",
          filename: `Proposal_${proposal.proposal_number || "draft"}_Executed.pdf`,
          storage_path: `proposals/${id}/signed_proposal.pdf`,
