...
export const DEFAULT_PIS_SECTIONS: RfiSectionConfig[] = [
  ...
  },
  {
    ...
    description: "Check 'Not known yet' if the party hasn't been selected",
    fields: [
      { id: "gc_heading", label: "General Contractor", type: "heading" },
      { id: "gc_tbd", label: "Not known yet", type: "checkbox", width: "full" },
      ...
      { id: "tpp_tbd", label: "Not known yet", type: "checkbox", width: "full" },
      ...
      { id: "sia_tbd", label: "Not known yet", type: "checkbox", width: "full" },
...
export function useRfiByToken(token: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("rfi_requests" as any)
        .select("*, properties(*), projects(building_owner_name, gc_company_name, gc_contact_name, gc_phone, gc_email)")
      ...
      return {
        ...
        project: projects as { building_owner_name: string | null; gc_company_name: string | null; gc_contact_name: string | null; gc_phone: string | null; gc_email: string | null } | null,
      };
    },
    enabled: !!token,
  });
