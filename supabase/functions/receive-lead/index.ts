...
import { isRateLimited, timingSafeEqual } from "../_shared/timingSafeEqual.ts";
...
serve(async (req) => {
  ...
  if (await isRateLimited(supabase, `receive-lead:${clientIp}`, 5, 60)) {
  ...
  // Shared secret check — constant-time compare to avoid timing oracles.
