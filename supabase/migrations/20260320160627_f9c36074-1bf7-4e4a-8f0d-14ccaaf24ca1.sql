...
DECLARE
  ...
  pis_owner_company text;
  pis_owner_person text;
  pis_owner_combined text;
BEGIN
  IF NEW.status = 'submitted' AND NEW.project_id IS NOT NULL 
    ...
    -- Combine owner company + person name: "Company — Person" or whichever exists
    pis_owner_company := NULLIF(TRIM(resp->>'applicant_and_owner_owner_company'), '');
    pis_owner_person := NULLIF(TRIM(resp->>'applicant_and_owner_owner_name'), '');
    
    IF pis_owner_company IS NOT NULL AND pis_owner_person IS NOT NULL THEN
      pis_owner_combined := pis_owner_company || ' — ' || pis_owner_person;
    ELSE
      pis_owner_combined := COALESCE(pis_owner_company, pis_owner_person);
    END IF;
    ...
    UPDATE public.projects SET
      ...
      building_owner_name = COALESCE(pis_owner_combined, building_owner_name),
    ...
    -- Always sync PIS owner to property record (supersedes PLUTO data)
    IF pis_owner_combined IS NOT NULL AND proj_property_id IS NOT NULL THEN
      UPDATE public.properties SET
        owner_name = pis_owner_combined,
        updated_at = now()
