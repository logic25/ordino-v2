...
-- ============================================================
-- Batch C: Funnel notifications (payment_received, invoice_overdue, lead_converted)
-- ============================================================

-- 1) payment_received: AFTER INSERT/UPDATE trigger on invoices when status transitions to 'paid'
...
BEGIN
  ...
  v_amount := COALESCE(NEW.payment_amount, NEW.total_due, 0);
  v_invoice_label := COALESCE(NEW.invoice_number, 'Invoice');

  IF NEW.project_id IS NOT NULL THEN
    SELECT assigned_pm_id, name INTO v_pm_id, v_proj_name
    FROM public.projects WHERE id = NEW.project_id;
  END IF;

  -- Build recipient set: project PM + all accounting users in company
  FOR v_recipient IN
    SELECT DISTINCT u FROM (
      SELECT v_pm_id AS u WHERE v_pm_id IS NOT NULL
      UNION
      SELECT p.id
      FROM public.user_roles ur
      JOIN public.profiles p ON p.user_id = ur.user_id AND p.company_id = NEW.company_id
      WHERE ur.role = 'accounting'::app_role
        AND (ur.company_id = NEW.company_id OR ur.company_id IS NULL)
    ) s
    WHERE u IS NOT NULL
  LOOP
    INSERT INTO public.notifications (company_id, user_id, type, title, body, link, project_id)
    VALUES (
      NEW.company_id, v_recipient, 'payment_received',
      'Payment received: ' || v_invoice_label,
      to_char(v_amount, 'FM$999,999,990.00') || ' received'
        || COALESCE(' for ' || v_proj_name, '') || '.',
...
-- 2) lead_converted: re-create convert_lead_to_proposal with a wrapped notification
CREATE OR REPLACE FUNCTION public.convert_lead_to_proposal(
  _lead_id uuid,
  _proposal jsonb DEFAULT '{}'::jsonb
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_client_id IS NULL THEN
    INSERT INTO public.clients (company_id, name, client_type, lead_owner_id)
    VALUES (v_lead.company_id, v_company_name, v_lead.client_type, v_lead.assigned_to)
...
REVOKE ALL ON FUNCTION public.convert_lead_to_proposal(uuid, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.convert_lead_to_proposal(uuid, jsonb) TO authenticated;

-- 3) Helpful index for overdue cron existence check
CREATE INDEX IF NOT EXISTS idx_notifications_type_link
  ON public.notifications(type, link)
