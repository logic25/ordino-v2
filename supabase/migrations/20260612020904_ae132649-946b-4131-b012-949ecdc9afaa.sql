-- Atomic lead → proposal conversion. Mirrors sign_proposal pattern.
-- One transaction: find-or-create client (+ primary contact), write lead.client_id + advance stage,
-- insert minimal proposal carrying all party metadata and lead_id back-link.
...
BEGIN
  ...
  -- 1) Resolve client: reuse existing link, else normalized name match, else create.
  ...
  -- 2) Insert proposal (minimal — items/milestones added via normal edit flow)
  ...
  -- 3) Write back lead linkage + advance stage atomically
  UPDATE public.leads
...
-- Helpful index for the back-link lookup
