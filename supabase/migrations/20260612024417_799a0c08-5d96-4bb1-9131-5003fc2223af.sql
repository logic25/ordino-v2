...
BEGIN
  ...
  (
    SELECT 'lead'::text, l.id,
           l.full_name::text,
           COALESCE(l.company, l.contact_email, l.contact_phone)::text,
           ('/bd/leads/' || l.id::text)
    FROM public.leads l
  ...
  (
    SELECT 'proposal'::text, p.id,
           COALESCE(NULLIF(p.proposal_number, ''), p.title, 'Proposal')::text,
           COALESCE(p.client_name, p.status::text)::text,
           ('/proposals?proposal=' || p.id::text)
  ...
  UNION ALL
  (
    SELECT 'client'::text, c.id, c.name::text,
           COALESCE(c.email, c.phone)::text,
           ('/clients?client=' || c.id::text)
  ...
  (
    SELECT 'project'::text, pr.id,
           COALESCE(NULLIF(pr.project_number, ''), pr.name, 'Project')::text,
           COALESCE(pp.address, pr.status::text)::text,
           ('/projects/' || pr.id::text)
  ...
  UNION ALL
  (
    SELECT 'property'::text, pp.id, pp.address::text,
           COALESCE(pp.borough, pp.bin)::text,
           ('/properties/' || pp.id::text)
